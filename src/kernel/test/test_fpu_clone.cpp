...
// child Input: Parent task uses x87 (finit + fldl pi), becomes fpu_owner; then
// clone Expect: child->fpu_used == true, child FXSAVE tag word shows ST0
// non-empty Depends: TaskControlBlock::clone, kernel::Scheduler, arch::fxsave
JARVIS_TEST(fpu_clone_copies_state, "PRE: none | POST: none") {
    ...
    JARVIS_ASSERT_FMT(fpu_owner_val == parent,
                      "fpu_owner (%p) != parent (%p) after FINIT/FLDL",
                      (void *)fpu_owner_val, (void *)parent);
    ...
    JARVIS_ASSERT_FMT(child->fpu_used, "child fpu_used not set after clone");
