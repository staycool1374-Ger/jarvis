...
class Fat32Partition {
  public:
    ...
    const Fat32Bpb &bpb() const {
        return bpb_;
    }
    uint64_t partition_lba() const {
        return partition_start_;
    }
    block::BlockDevice &device() const {
        return device_;
    }
...
bool find_fat32_in_mbr(block::BlockDevice &device, uint64_t &partition_lba_out);
