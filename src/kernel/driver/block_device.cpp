...
/// @brief Construct a mock device wrapping an existing buffer (optionally
/// read-only).
...
MockBlockDevice::MockBlockDevice(uint8_t *external_data, uint64_t sector_count_,
                                 bool read_only)
...
bool MockBlockDevice::read_sector(uint64_t lba, uint8_t *buffer) {
    if (!buffer || !data_ || lba >= sector_count_)
        return false;
...
bool MockBlockDevice::write_sector(uint64_t lba, const uint8_t *buffer) {
    if (!buffer || !data_ || lba >= sector_count_ || read_only_)
        return false;
