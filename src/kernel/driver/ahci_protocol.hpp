...
///        Shared between kernel-mode driver and userspace (ring-3) driver server.
///        No kernel dependencies — standalone portable header.
...
constexpr uint32_t SSTS_DET_OFFLINE     = 0x04; // Device presence, phy offline (sleep)
constexpr uint32_t SSTS_SPD_MASK = 0x000000F0; // Negotiated Speed (bits 4-7)
constexpr uint32_t SSTS_IPM_MASK = 0x00000F00; // Interface Power Management (bits 8-11)
...
struct CmdHeader {
    ...
    uint16_t    attrs;      // Attributes (bits 5-15: A=ATAPI, W=Write, C=ClearBusy, P=Prefetch, R=Reset)
    // DW1
...
struct PrdHbaEntry {
    ...
    uint32_t    reserved;
    uint32_t    byte_count; // Byte count (22 bits: bits 0-21), bit 31 = Interrupt on Completion
} __attribute__((packed));
