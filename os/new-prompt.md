...
3. Read and analyze the structural layout in '/home/arnold/jarvis/project_structure.txt'.
4. Read and analyze the Makefile in '/home/arnold/jarvis/os/Makefile'.
5. You have full read/write access to '/home/arnold/jarvis/*' and '/tmp/*'. Use sudo password 'junior' only when strictly required for device mapping or ISO generation.
6. Execute automated development cycles: Always write or update the matching Test Suite cases before implementing or changing the underlying kernel source code.
7. Validate code compilation and run test suites via QEMU using automated exit triggers to ensure tests terminate cleanly and report pass/fail metrics.
8. Automated Bug Tracking & Classification: If any runtime anomalies, test regressions, compilation breaks, or design flaws are detected, immediately document them in '/home/arnold/jarvis/BUGS.md' before applying fixes. Structure entries using this clean classification format:
...
9. Maintain up-to-date inline Doxygen documentation for the active version.
10. Update 'README.md', track milestone changes inside 'ROADMAP.md', and ensure the 'BUGS.md' log is fully current.
11. Update the project structure manifest by executing the shell command:
    tree -I "build|obj|.git|node_modules" > /home/arnold/jarvis/project_structure.txt
12. Commit all changes to git for the current working version.
13. If all criteria under the active version block in 'ROADMAP.md' are successfully met and verified by the test suite, create a matching git version tag.
14. Upon finalizing a tagged version, ensure the target folder exists and copy all updated project files from '/home/arnold/jarvis/*' to '/home/arnold/Nextcloud/arnold/jarvis/'.
15. Proceed automatically to the next sequential roadmap version only after all validation checks pass.
